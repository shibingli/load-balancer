...
func TestSmoothWeightedRoundRobin(t *testing.T) {
	...
	item := lb.Select()
	if item != nil {
		t.Fatalf("swrr expected nil, actual %s", item)
	}
...
func TestSmoothWeightedRoundRobin_C(t *testing.T) {
	...
	if atomic.LoadInt64(&a) <= 400000 {
		t.Fatal("swrr wrong: a", atomic.LoadInt64(&a))
	}
	if atomic.LoadInt64(&b) <= 40000 {
		t.Fatal("swrr wrong: b", atomic.LoadInt64(&b))
	}
	if atomic.LoadInt64(&c) <= 300000 {
		t.Fatal("swrr wrong: c", atomic.LoadInt64(&c))
	}
	if atomic.LoadInt64(&d) != 0 {
		t.Fatal("swrr wrong: d", atomic.LoadInt64(&d))
	}
	if atomic.LoadInt64(&a)+atomic.LoadInt64(&b)+atomic.LoadInt64(&c) != 1000000 {
		t.Fatal("swrr wrong: sum")
	}
}
